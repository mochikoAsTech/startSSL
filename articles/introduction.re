...
SSLは好きですか？それとも怖いですか？本著を書くまで、筆者は「ちゃんと分かっているとは言えないので、SSLのことを迂闊に話題に出すのはなんだか怖い」と感じていました。

SSLは、エンジニアのごく身近なところにあります。たとえば「サイトをフルSSL化する」「SSL証明書を更新する」のような形で、誰しも一度はSSLと関わりをもったことがあるのではないでしょうか。しかし関わる機会が多い割に、「ちゃんと分かっているか」と言われるとちょっと自信がない、そんなエンジニアは、筆者を含め、きっと一定数いることと思います。

Googleが2014年ごろからさかんに提唱している「HTTPS everywhere」のもと、ウェブサイトの評価基準は「HTTPで100点、HTTPSにするとさらに加点」から、「HTTPSで100点、できていなければ減点」に変わってきました。ますますSSLと関わる機会が増えてきて、心のどこかで「SSL、ちゃんと分かりたいなぁ…」と思っている。本著はそんな人のための一冊です。

理解度は「分かった！」「分かってなかった…」をくり返して、段々と上がっていくものです。まずは本著で、一緒に最初の「分かった！」まで進んでみましょう。

ちなみに本著「SSLをはじめよう」（以下SSL本）では、「DNSをはじめよう」（以下DNS本）で購入したドメイン名を使用します。読み進んでいくと第2章辺りで「ここで事前にあく抜きしておいた筍を取り出します」と言われて、「は？あく抜きとかいつしてたの？！」という状態になりますので、「DNSは興味ないし面倒くさいんだけど…」という方も、できればDNS本をお読みいただいて、筍の下ごしらえ（＝ドメイン名の購入）を済ませた状態でSSL本を開いてみてください。きっとその方が美味しくお召し上がりいただけます。なおSSL本の第1章は、DNS本を読んでいなくても問題ない内容ですので、とりあえずそのまま読み進めていただいても構いません。
...
DNS本、AWS本、そしてSSL本のはじめようシリーズは、「サーバやインフラは怖いものではなくすごく楽しいものなんだよ」ということを、かつての私のようなインフラ初心者へ伝えたくて書いたシリーズです。
...
本著では実際にサーバを立ててSSL証明書の設置を行い、HTTPSのサイトを作ってみます。手を動かして試してから仕組みを学べるので理解がしやすく、インフラ初心者でも安心して読み進められる内容です。Oracle Cloudの無料枠の中でサーバを立てて使用しますので、サーバ代はかかりません。SSL証明書代のみ、1,100円（税込）かかります。

また実際にやってしまいがちな失敗、トラブルをとり上げて、

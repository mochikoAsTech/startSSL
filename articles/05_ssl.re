...
//footnote[rsa][もともとは認証だけでなく鍵交換もRSAで行なわれていましたが、RSAによる鍵交換は、一度秘密鍵が盗まれてしまうと、過去のやりとりもさかのぼって全ての暗号データを復号可能という問題がありました。そのためTLS1.3ではRSA鍵交換は廃止されています]
 ...
 1. クライアント側でSSL証明書を受け取って次の3つを行なう
   ...
   1. SSL証明書本体のSANまたはCNに記載されているFQDN@<fn>{fqdn}と、リクエスト先のFQDN（bank.example.com）が同一であることを確認
...
ブラウザでHTTPSから始まるURLを開くと、ウェブページはサーバから自分のパソコンまで、HTTPSで暗号化されて届きます。しかし取得したページのHTMLに、<img src="http://www.example.com/images/top.png">のような、絶対パスで画像を指定する<img>タグが含まれていると、その画像ファイルは暗号化されていないHTTPで送られてきます。

つまり、せっかくウェブページをHTTPSで開いても、そのウェブページのHTMLソースの中で、CSSや画像ファイルの指定をhttp://から始まる形式にしていたり、YouTubeなどのコンテンツをhttp://から始まる形式で埋め込んでいると、ブラウザは鍵マークの代わりにiマークを表示して「ページの一部が暗号化されていないので、安全ではありません」と警告を出すのです。
...
あるいは、「⁠画像はウェブページとは別のサーバにあるので、ドメインから書かなければいけない」という場合は、<img>タグを<img src="//image.example.com/images/top.png"> のように書くことで、先ほどと同じように、ページをHTTPで開いたときは画像もHTTPで表示でき、HTTPSで開いたときは画像もHTTPSで表示できます。その結果、鍵マークは壊れません。
...
SSL証明書と中間CA証明書はサーバに設置されているため、ブラウザでサイトを開くと、ウェブページと一緒にパソコンへ届けられます。これでAさん、Bさん、Cさんの身元は保証されましたが、ではDさんこと一番下の中間CA証明書2を発行した「GTE CyberTrust Global Root」の身元を証明する「ルート証明書」はどこにあるのでしょうか。

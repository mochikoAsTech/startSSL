...
またHTTPSにすることで、HTTP/2というHTTPプロトコルの次期バージョンを利用できる@<fn>{browserNeeds}ようになります。SSLのオーバーヘッドは特にハンドシェイクに集中しているため、長期にわたって接続したままで、リクエストとレスポンスを多重化できるHTTP/2と組み合わせると、ハンドシェイクの回数が減る分だけ接続効率が上がります。ページの作りによってもちろん向き不向きがあり、HTTPSにしたことで表示速度が落ちるケースもあると思います。ですが、たとえばサイズが小さな画像を大量に表示するようなページ（@<img>{startSSL_124}）@<fn>{httpVsHttps}は、HTTP/2の恩恵を受けやすく、速度の向上が期待できます。

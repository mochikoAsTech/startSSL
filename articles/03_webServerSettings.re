...
//image[startRLogin13][このエラーが表示されたら「ホスト名（サーバーIPアドレス）」のIPアドレスを確認しよう][scale=0.6]{
//}
...
ウェブサーバの準備ができたので、HTTPSのサイト用にドメイン名を用意します。「自分のドメイン名？そんなの持ってないよ！」という人は、先に「DNSをはじめよう」（@<img>{startDNS}）で、ドメイン名を買ってからこの先へ進むようにしてください。

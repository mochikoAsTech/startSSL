...
［ホーム・リージョン］は［日本東部(東京)］を選択してください。Oracle Cloudは世界の各地域にデータセンターを所有しており、サーバはそのデータセンターの中で元気に動いています。この［ホーム・リージョン］とは、@<ttb>{各地域の中でどこを使うか？を指定するもの}です。ウェブサイトにアクセスするとき、パソコンのある場所からサーバまで物理的に距離が遠いと、それだけ通信にも時間がかかって応答時間も遅くなります。日本国内向けにウェブサイトを開設する場合は、基本的にこの［日本東部(東京)］のリージョンを選びましょう。ただしOracle Cloudのサービスによってはまだ東京リージョンが使えないものもあります。その場合は次点として「米国東部(アッシュバーン)」を選択してください。
...
迷惑メールの設定を確認して何回か試して、それでもSMSが届かなかったら、ページ下部の［サポートが必要ですか。ご連絡ください: チャット・サポート］からサポートにチャットで問い合わせてみましょう。残念ながら英語でしか対応してもらえませんが、"I am trying to register on Oracle Cloud. But I can't receive SMS. What should I do?"（アカウント登録しようとしてるけどSMSが届かないの、どうしたらいい？）という感じで聞いてみると、「じゃあ登録情報をこのチャットで教えて。そうしたらこちらでコードを発行して、このチャットで伝えてあげる」（意訳）という感じでサポートしてもらえます。

=== パスワードと支払情報の登録

正しいコードが入力できたら、［パスワードの入力］が表示されます。［パスワード］と［パスワードの確認］を入力して、［次: 支払情報］をクリックします。（@<img>{startSSL_35}）

//image[startSSL_35][パスワードを入力して［次: 支払情報］をクリック][scale=0.8]{
//}

パスワードを入力すると、今度は［支払情報］のページが表示されます。（@<img>{startSSL_36}）繰り返しお伝えしているとおり、Oracle CloudにはAlways Freeという無料枠があります。本著では基本的にこの無料枠の範囲内でOracle Cloudを使っていくつもりですが、それでもクレジットカードは登録しておく必要があります。
...
これでアカウント登録の手続きはおしまいです。［アカウントの設定が完了するまでお待ちください。］と表示（@<img>{startSSL_40}）されます。準備が整うとサインイン画面にリダイレクトされますが、この［アカウントの設定が完了するまでお待ちください。］の画面でかなり時間がかかるので、一度ブラウザを閉じてしまって構いません。頑張った自分を褒めて、一旦休憩にしましょう。

...
まずは、サーバを立てるときに必要なターミナルというソフトを準備して、キーペアの作成を行ないます。Windows、Macの順番で手順を説明します。
...
Oracle Cloudは他のクラウドに比べて価格が安いのが特徴のひとつです。どれくらい安いのか、AWSとOracle Cloudでサーバの価格を比較してみましょう。
...
//footnote[mitmMac][yesと打った理由は、P60の注釈で説明しています]
...
先ほどサーバの中にあるファイアウォールの設定を変更して、HTTPとHTTPSが通れるようにしましたが、実はサーバの中だけでなく、サーバに辿り着く手前にも、もう1つファイアウォールがいます。サイトが表示されなかったのは、「ウェブページを見せて！」というリクエストが、サーバの手前のファイアウォールで阻まれていたためなのです。サーバの手前にあるファイアウォールにも穴を空けて、HTTPとHTTPSがサーバまでたどり着けるようにしましょう。
...
HTTPでサイトを見ることができましたね。では同じドメイン名をHTTPSで開いてみるとどうでしょう？ブラウザで「https://ssl.自分のドメイン名」を開いてみると、［正常に接続できませんでした］と表示（@<img>{startSSL_85}）されました。

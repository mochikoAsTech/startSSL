...
この章では、ウェブサーバとドメイン名を用意して、まずはHTTPでサイトを見てみましょう。
...
「opc@startsslinstance」と表示されたら無事サーバに入れています。おめでとうございます！今後は、いまやったのと同じやり方をそのまま繰り返せば、サーバにログインできます。

== NGINXをインストールしよう

それでは必要なミドルウェアをインストールしていきましょう。最初にrootになっておいてください@<fn>{prompt}。インストールするときはyum（ヤム）というコマンドを使います。

//footnote[prompt][以降は、例として書いてある部分のプロンプトが、「$」だったらopcのような一般ユーザで実行、「#」だったらrootでコマンドを実行してください。例に「$」や「#」が書いてあってもターミナルで「$」や「#」を自分で入力する必要はありません]

//cmd{
$ sudo su -
//}

先ずはyumで色々アップデートしておきましょう。Windowsアップデートみたいなものです。画面にたくさん文字が流れて、少し時間がかかりますが、最後に［Complete!］と表示されたら問題なく完了しています。（@<img>{startSSL_66}）ちなみに-yオプションはYESを意味するオプションです。-yオプションをつけないで実行すると「これとこれを更新するけどいい？ダウンロードサイズとインストールサイズはこれくらいだよ」という確認が表示され、yと入力してEnterキーを押さないと更新されません。
...
ウェブサーバも立てたし、サーバの中のファイアウォールに穴も空けました。これで準備完了！サーバを立てたときにメモした［パブリックIPアドレス］を、ブラウザで開いてみましょう。するとしばらくぐるぐるした後で、［接続がタイムアウトしました］と表示されてしまいました。（@<img>{startSSL_77}）
...
ウェブサーバを立てて、HTTPでサイトを表示できたので、続いてはHTTPSのサイト用にドメイン名を用意します。「自分のドメイン名？そんなの持ってないよ！」という人は、先に「DNSをはじめよう」@<fn>{boothDNS}で、ドメイン名を買ってからこの先へ進むようにしてください。

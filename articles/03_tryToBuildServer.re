...
Windowsのパソコンを使っている方は、サーバを立てる前に「ターミナル」と呼ばれる黒い画面のソフトをインストールしておきましょう。サーバに接続するときにはこのターミナルを使います。ターミナルのソフトには色々な種類がありますが、本著ではRLogin@<fn>{rlogin}を使用します。RLoginのインストール方法は「AWSをはじめよう」で説明していますので、本著では省略します。

...
なお今回はウェブサーバにSSL証明書を設置しましたが、ウェブサーバの手前にロードバランサを置いて、そこにSSL証明書を設置し、SSLターミネーションを行う方法もあります。その場合、SSLで通信するのはエンドユーザのパソコンからロードバランサーまでで、ロードバランサーからウェブサーバはHTTPで通信するのが一般的です。次のようなメリットがあります。

- 暗号化や復号の処理を行う終端がロードバランサになるので、ウェブサーバのスペックが低くてもよいパフォーマンスが得られる
- 証明書の取得や更新が自動で行われる
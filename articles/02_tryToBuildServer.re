...
次はキーペアを作成します。Windowsの方は、起動したRLoginで［新規(N)］をクリックします。
...
無事にSSHのキーペア（秘密鍵と公開鍵）が作成されて、［認証キーリスト］に［startSSLKey］が表示されました。［公開鍵］をクリックしてください。（@<img>{downloadRLogin04}）
...
サーバが起動すると、表示が緑色の［実行中］に変わります。おめでとうございます！これでサーバが立てられました！
...
ところで、いま立てた「VM.Standard.E2.2」というシェイプのサーバをまるまる1ヶ月使ったら、一体いくら分になるのでしょう？うっかり$300を超えてしまわないか、ちょっと心配なので計算してみましょう。
...
無事にサーバが立ち上がったら、サーバのIPアドレスを確認しておきましょう。
...
それでは次の章で、メモしたIPアドレスを使ってサーバに入ってみましょう。

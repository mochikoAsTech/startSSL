...
SSL証明書は、関わってくる登場人物が多いので、最初に登場人物をご紹介します。
...
どこに何を置いたか分からなくなってしまわないよう、これらの登場人物が集まる場所として、サーバの中で@<code>{/etc/nginx/}の下に@<code>{ssl}というディレクトリを作っておきましょう。@<fn>{error}
...
先ず最初に作成するのが秘密鍵@<fn>{secret}です。秘密鍵は「.key」や「.pem」@<fn>{pem}という拡張子で作成されることが多いです。秘密鍵は名前のとおり「秘密」にすべきです。つまり、限られた人だけが触れるように管理すべきです。決してメールに添付して送ったり、サーバ内で誰でも見られる@<code>{/tmp/}以下に置いたりしてはいけません。
...
次のopensslコマンドをたたいて、秘密鍵を生成しましょう。左から順に「opensslコマンドで、鍵アルゴリズムがRSAの秘密鍵を作る。作った秘密鍵は/etc/nginx/ssl/以下にstartssl.keyというファイル名で保存する。鍵の長さは2048ビット。」という意味です。できあがった秘密鍵は、chmodコマンドで読み書き権限を厳しくしておきます。
...
opensslコマンドで秘密鍵を作るとき、-aes128や-aes256というオプションを付けると、パスフレーズを聞かれて、指定の暗号方式で暗号化された秘密鍵が出力されます。@<fn>{option}［Enter pass phrase:］や［Verifying - Enter pass phrase:］と表示された際に、いくらキーボードをたたいて入力しても、黒い画面上は何も表示されません。ですが、ちゃんと文字入力はできているので大丈夫です。入力を間違えたら、Backspaceキーで消して書き直すこともできます。
...
左から順に、「opensslコマンドで、CSRを作る。新しく作る。元になる秘密鍵はstartssl.key。CSRはstartssl.csrというファイル名で作る。」という意味です。
...
CSRをcatコマンドで表示して、「-----BEGIN CERTIFICATE REQUEST-----」から「-----END CERTIFICATE REQUEST-----」までをコピーしたら、メモ帳にペーストしておきましょう。メモしたCSRはこの後使用します。
...
//footnote[fujissl][検索すると色んな種類のSSL証明書が出てきますが、値段によって何が違うのかについては、@<chapref>{05_ssl}で後述します。本著ではFujiSSLという安価な証明書を取得します]
...
認証方式へ進みます。これは「CSRのコモンネームで指定したドメイン名が、あなたの持ち物であることをどうやって証明しますか？」ということを聞かれています。対象のドメイン名で、メールを受信してURLを踏む（メール認証）か、指定された内容のリソースレコードをDNSで追加する（DNS認証）か、サイトに指定された内容のファイルをアップする（ファイル認証）か、いずれかの方法で「このドメイン名（筆者ならssl.startdns.fun）は私の持ち物です」ということを証明しなければいけません。
...
［CommonName=ssl.自分のドメイン名］から始まる件名のメール（@<img>{startSSL_102}）がすぐに届きました。CSRのコモンネームで指定したドメイン名が［ssl.startdns.fun］の場合、［startdns.fun］のTXTレコードを追加して、メールに書いてある値を設定するよう書いてあります。

//image[startSSL_102][追加すべきTXTレコードの値がメールで届いた][scale=0.8]{
//}

例えばネームサーバが「お名前.com」なら、DNS設定の画面で次のようにTXTレコードを追加（@<img>{startSSL_103}）します。今回はサブドメインを含まないドメイン名を追加するので、［ホスト名］には何も入力しません。［VALUE］には、メールに書いてあった値をそのままコピー＆ペーストします。

//image[startSSL_103][「自分のドメイン名」のTXTレコードを追加する][scale=0.6]{
//}

TXTレコードが追加されたかどうかは、次のdigコマンドで確認できます。digコマンドをたたいた結果、メールに書いてあった値が返ってくればTXTレコードは設定できています。
...
どちらも必要なものなので、SSL証明書と中間CA証明書の2つをサーバにアップロードしましょう。
...
左側にあなたのパソコンのファイルが表示されていて、右側にサーバのファイルが表示されています。左側で、デスクトップに展開したフォルダ@<fn>{folder}を選択したら、［ssl.自分のドメイン名.crt］と［ca-bundle.ca］を右側にドラッグ＆ドロップ（@<img>{startSSL_112}）してください。これでサーバの「/home/opc」にファイルがアップロードされます。
...
右側に［ssl.自分のドメイン名.crt］と［ca-bundle.ca］がアップされたら、×を押してファイル転送の画面は閉じて構いません。（@<img>{startSSL_113}）
...
Macを使っている方は、ターミナルを起動してください。sshコマンドではなく、scpコマンドを使って、Macの中にある［ssl.自分のドメイン名.crt］と［ca-bundle.ca］を、サーバへアップロードします。［ZIPを展開したフォルダ］と［パブリックIPアドレス］の部分は、ご自身のものに書き換えてください。
...
これでWindowsの方も、Macの方も、サーバにSSL証明書と中間CA証明書をアップロードできました。それではサーバにSSHでログインして、先ほどアップロードした［ssl.自分のドメイン名.crt］と［ca-bundle.ca］が、ちゃんと「/home/opc/」以下に存在していることを確認しましょう。
...
暗号スイートとは、「認証はRSAで、鍵交換はECDHE、暗号化アルゴリズムはAESを使う」のように、セキュリティの各パラメータをセットにしたものです。ファーストフード店の「チキンクリームポットパイ＋ポテト＋コーラSのセット」や「オリジナルチキン＋ビスケット＋アイスティーSのセット」みたいなものだと思ってください。どんなセットがあるのか、一覧はIANAのページ@<fn>{iana}で確認できます。
...
クライアントが希望する暗号スイート群をサーバに送ったときに、サーバ側で、ここで登録した暗号スイートを上から順に探していき、共通のものが見つかれば、その暗号スイートを使用します。つまりクライアントが「フィレオフィッシュ＋ポテト＋烏龍茶のセット！なければキッズナゲット＋ビスケット＋烏龍茶のセット！」と希望しても、サーバ側のssl_ciphersディレクティブにそのセットがなければ、接続できずに［クライアントとサーバーで、共通の SSL プロトコル バージョンまたは暗号スイートがサポートされていません。］と表示されてしまいます。（@<img>{startSSL_123}）

//image[startSSL_123][共通の暗号スイートが見つからず、接続できなかった様子][scale=0.6]{
//}

そのためssl_ciphersディレクティブに、セキュリティを重視して強度の高い暗号スイートだけを書くのか、それとも古い端末との互換性を重視して脆弱な暗号スイートも含めて書くのかは、安全性と可用性（対応端末の多さ）のバランスを考えて決めていくことになります。本来はサイトの運営者が自分で考えて判断すべきことではありますが、次のように設定値の参考を得ることもできます。
...
なお本著ではウェブサーバにSSL証明書を設置しましたが、ウェブサーバの手前にロードバランサを置いて、そこにSSL証明書を設置して、SSLターミネーションを行なう方法もあります。その場合、HTTPSで通信するのはエンドユーザのパソコンから終端となるロードバランサーまでで、ロードバランサーとウェブサーバの間はHTTPで通信するのが一般的です。
...
//footnote[license][ちなみに本著で扱ったFujiSSLでは、取得した1枚のSSL証明書を複数台のウェブサーバに設置しても構いません。ですが認証局やSSL証明書によっては、「1台にしか設置できません。2台以上に設置する場合は台数分だけ購入してください」というケースもありますので、購入前にライセンス形態を確認しておきましょう]

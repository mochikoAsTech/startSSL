...
最初に作成するのが秘密鍵@<fn>{secret}です。秘密鍵は「.key」や「.pem」@<fn>{pem}という拡張子で作成されることが多いです。秘密鍵は名前のとおり「秘密」にすべきです。つまり、限られた人だけが触れるように管理すべきですので、決してメールに添付して送ったり、サーバ内で誰でも見られる@<code>{/tmp/}以下に置いたりしてはいけません。
...
それでは次のopensslコマンドをたたいて、秘密鍵を生成しましょう。左から順に「opensslコマンドで、鍵アルゴリズムがRSAの秘密鍵を作る。作った秘密鍵は/etc/nginx/ssl/以下にstartssl.keyというファイル名で保存する。鍵の長さは2048ビット。」という意味です。できあがった秘密鍵は、chmodコマンドで読み書き権限を厳しくしておきます。
...
［認証方式を選択］へ進みます。これは「CSRのコモンネームで指定したドメイン名が、あなたの持ち物であることをどうやって証明しますか？」ということを聞かれています。対象のドメイン名で、メールを受信してURLを踏む（メール認証）か、指定された内容のリソースレコードをDNSで追加する（DNS認証）か、指定された内容のファイルをウェブサイトにアップする（ファイル認証）か、いずれかの方法で「このドメイン名（筆者ならssl.startdns.fun）は私の持ち物です」ということを証明しなければいけません。
...
［CommonName=ssl.自分のドメイン名］から始まる件名のメール（@<img>{startSSL_102}）がすぐに届きます。例えばCSRのコモンネームで指定したドメイン名が［ssl.startdns.fun］だった場合、ベースドメイン名である［startdns.fun］のTXTレコードを追加して、メールに書いてある値を設定するよう書いてあります。
...
//image[mozilla][環境や設定を選ぶだけで適した設定ファイルが出力されるMozillaのサイト][scale=0.6]{
//}
...
証明書を設置して、NGINXの設定ファイルを修正し、NGINXの再起動もしました。それではブラウザで「https://ssl.自分のドメイン名」を開いてみましょう。鍵マーク付きでHTTPSのページが表示されるはずです。（@<img>{startSSL_114}）
...
なお本書ではウェブサーバにSSL証明書を設置しましたが、ウェブサーバの手前にロードバランサを置いて、そこにSSL証明書を設置する「SSLターミネーション」という方法もあります。その場合、HTTPSで通信するのはエンドユーザのパソコンから終端となるロードバランサーまでで、ロードバランサーとウェブサーバの間はHTTPで通信するのが一般的です。

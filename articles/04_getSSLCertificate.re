...
次のopensslコマンドを叩いて、秘密鍵を生成しましょう。左から順に「opensslコマンドで、鍵アルゴリズムがRSAで、鍵の長さは2048ビットで、/etc/nginx/ssl/以下にstartssl.keyというファイル名で秘密鍵を作って」という意味です。できあがった秘密鍵は、chmodコマンドで読み書き権限を厳しくしておきます。

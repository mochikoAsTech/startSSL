...
//footnote[fujissl][検索すると色んな種類のSSL証明書が出てきますが、値段によって何が違うのかについては、@<chapref>{05_ssl}で後述します。本書ではFujiSSLという安価な証明書を取得します]
...
なお本書ではウェブサーバにSSL証明書を設置しましたが、ウェブサーバの手前にロードバランサを置いて、そこにSSL証明書を設置して、SSLターミネーションを行なう方法もあります。その場合、HTTPSで通信するのはエンドユーザのパソコンから終端となるロードバランサーまでで、ロードバランサーとウェブサーバの間はHTTPで通信するのが一般的です。

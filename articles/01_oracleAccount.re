...
ウェブサイトを公開するにはサーバが必要です。そしてサーバを立てるには、普通はお金がかかります。ですがオラクルがやっている「Oracle Cloud（オラクル クラウド）」というサービスなら、なんと有効期限がなく、@<ttb>{ずっと無料で使えるAlways Free}という枠があります。Always Freeの範囲内であれば、サーバも無料で立てられるので、本著ではそれを使いましょう。
...
クラウドはOracle Cloudだけではありません。かの有名なAWSことAmazon Web Servicesや、GoogleのGoogle Cloud Platform@<fn>{gcp}、MicrosoftのAzure（アジュール）@<fn>{azure}、その他にも国内クラウドとしてさくらインターネットがやっているさくらのクラウド@<fn>{sakura}、お名前.comでお馴染みGMOグループのGMOクラウド@<fn>{gmoCloud}などなど、たくさんのクラウドが存在しています。
...
しかし最近は、Alibaba CloudやTencent Cloudといった中国のクラウド事業者も追い上げを見せています。こうした新興のクラウドは、先を行くAWSを見て学んだ上で生まれてきただけあって、よりスマートな作りになっているのがいいところです。
...
//footnote[alwaysFree][Always Freeは期限なしでずっと無料ですが、利用できる範囲が決まっています。Always Freeの他に、30日間だけ有効な300ドル分の無償クレジットも付いてきますので、範囲外のサービスはそちらで試せます。詳細は@<href>{https://www.oracle.com/jp/cloud/free/}を確認してください]
...
Oracle Cloud Free Tierのページが表示されたら、［今すぐ始める（無償）］をクリックします。（@<img>{startSSL_28}）
...
［ホーム・リージョン］は［日本東部(東京)］を選択してください。Oracle Cloudは世界の各地域にデータセンターを所有しており、サーバはそのデータセンターの中で元気に動いています。この［ホーム・リージョン］とは、@<ttb>{各地域の中でどこを使うか？を指定するもの}です。

ウェブサイトにアクセスするとき、パソコンのある場所からサーバまで物理的に距離が遠いと、それだけ通信にも時間がかかって応答時間も遅くなります。日本国内向けにウェブサイトを開設する場合は、基本的にこの［日本東部(東京)］のリージョンを選びましょう。ただしOracle Cloudのサービスによってはまだ東京リージョンが使えないものもあります。その場合は次点として「米国東部(アッシュバーン)」を選択してください。
...
//footnote[upgrade][300ドル分の無償クレジットを使い切るか、あるいはアカウントを作ってから30日が経過すると、メールでお知らせが届きます。そこからさらに30日の間に有償アカウントへアップグレードしなければ、Always Freeのサービスを除いて、無償クレジットで作ったサーバやデータは削除されます。AWSのように、サーバを削除し忘れたまま無料期間が終わって、うっかり請求がきてしまった…という事態は、Oracle Cloudでは起きないので安心してください]

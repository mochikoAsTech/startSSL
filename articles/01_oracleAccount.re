...
実際にSSL証明書を取得して、HTTPSのサイトを作る、その下準備としてまずはアカウント作成からはじめましょう。
...
つまり、せっかくHTMLや画像でウェブサイトのコンテンツを作っても、それを載せておくウェブサーバがなければ、サイトはあなたのパソコンの中でしか見られず、インターネットで公開できないのです。@<fn>{startAWS}
...
ウェブサイトを公開するにはサーバが必要です。そしてサーバを立てるには、普通はお金がかかります。ですがオラクルがやっている「Oracle Cloud（オラクル クラウド）」というサービスには、なんと有効期限なしで@<ttb>{ずっと無料で使えるAlways Free}という枠があります。Always Freeの範囲@<fn>{alwaysFreePlan}内であれば、サーバも無料で立てられます。

//footnote[alwaysFreePlan][Always Freeの範囲については、@<href>{https://www.oracle.com/jp/cloud/free/}を確認してください]

さらにAlways Freeとは別に、30日間だけ有効な300ドル分の無償クレジットも付いてきますので、Always Freeの範囲外のサービスはそちらで試せます。本著ではこのAlways Freeと、無償クレジットの範囲内でOracle Cloudを利用していきます。

Oracle Cloudは名前のとおりオラクルがやっているクラウドですが、そもそもクラウドがなんだか分からないといまいちピンと来ないかもしれません。あなたが「ウェブサイト公開したいなぁ…だからサーバが必要だ！」と思ったとき、従量課金で@<ttb>{すぐに使えて性能や台数の増減も簡単にできるのがクラウド}です。

Oracle Cloudなら、ブラウザでぽちぽちとスペックを選んでいくだけで、すぐにサーバが使えます。
...
本著では次の2つを目的として、それに適したOracle Cloudで学びを進めていきたいと思います。
...
なお本著では、前述のとおりAlways Freeと300ドル分の無償クレジットという無料枠の範囲内で、Oracle Cloudを利用していきます。クレジットカードの登録は主に本人確認のために行なうもので、無料アカウントから有償アカウントへ手動でアップグレードしない限り、勝手に課金はされないので安心してください。
...
「Oracle Cloud へのサインアップ」と表示されたら、［電子メール・アドレス］と［国/地域］を入力して、使用条件を確認した上で［次］をクリックしましょう。（@<img>{startSSL_29}）後で分からなくならないように、登録した項目を@<table>{oracleAccount}にメモしておきましょう。
...
次は「アカウント詳細の入力」です。（@<table>{oracleAccountType}）今回は仕事ではなく個人での利用ですので［アカウント・タイプ］は［個人使用］を選択してください。［クラウド・アカウント名］には@<ttb>{任意のアカウント名}を入力します。［クラウド・アカウント名］には英字小文字と数字のみ使えます。記号や英字大文字は使えないので注意してください。筆者は@<code>{startdns01}にしました。この［クラウド・アカウント名］は、後で管理画面にサインインするときのアカウントURLになります。（@<img>{startSSL_30}）

//image[startSSL_30][［クラウド・アカウント名］には好きな名前を入力][scale=0.8]{
//}

［ホーム・リージョン］は［日本東部(東京)］を選択します。Oracle Cloudは世界の各地域にデータセンターを所有しており、サーバはそのデータセンターの中で元気に動いています。この［ホーム・リージョン］とは、@<ttb>{各地域の中でどこを使うか？を指定するもの}です。

ウェブサイトにアクセスするとき、パソコンのある場所からサーバまで物理的に距離が遠いと、それだけ通信にも時間がかかって応答時間も遅くなります。日本国内向けにウェブサイトを開設する場合は、基本的にこの［日本東部(東京)］のリージョンを選びましょう。ただしOracle Cloudのサービスによってはまだ東京リージョンで使えないものもあります。その場合は次点として「米国東部(アッシュバーン)」を選択してください。

この@<ttb>{［ホーム・リージョン］は後から変更ができません}。そしてAlways Freeのサービスは、選択した［ホーム・リージョン］においてのみ利用できます。

2020年2月現在、Oracle Cloudの［日本東部(東京)］のリージョンは人気が高く、Always Free向けのリソースが不足していてサーバが立てられない、という事態がしばしば起きています。［米国東部(アッシュバーン)］をホーム・リージョンにすれば、このリソース不足は回避できますので、Always Freeを確実に使いたい方は［米国東部(アッシュバーン)］を選択してください。本著ではAlways Freeが使えなかった場合は、300ドル分の無償クレジットを使用しますので、［日本東部(東京)］を選択して問題ありません。
...
//image[startSSL_34][コードの書かれたSMSが届いた][scale=0.4]{
//}
...
電話番号を入力したのにSMSが届かないときは、まず自分が契約している携帯キャリアの迷惑メール設定で、SMSを@<ttb>{スパムとしてはじく設定をしていないか}確認してみましょう。たとえば海外の事業者から送信されたSMSを拒否する設定になっていたり、海外からの着信を拒否する設定になっていると、SMSが届かないことがあるようです。@<fn>{smsFromOversea}

//footnote[smsFromOversea][「Oracle CloudのSMSは海外の事業者から届く」という確証がある訳ではないです。あくまでSMSが届かないときによくある話と思ってください]

ちなみに筆者の場合は、特に設定変更をせず同じ番号で2回試してみたのですが、最初は届かず、もう1回試してようやく届きました。
...
正しいコードが確認できると、［パスワードの入力］が表示されます。［パスワード］と［パスワードの確認］を入力して、［次: 支払情報］をクリックします。（@<img>{startSSL_35}）
...
繰り返しお伝えしているとおり、Oracle CloudにはAlways Freeという無料枠があります。本著ではこの無料枠の範囲内でOracle Cloudを使っていきますが、それでもクレジットカードは登録しておく必要があります。

なおページに記載されているとおり、この後、管理画面で［アカウントのアップグレード］という作業をして、有償アカウントへ切り替えない限り、請求は絶対に発生しません@<fn>{upgrade}ので安心してカード情報を登録してください。［クレジット・カード詳細の追加］をクリックします。
...
なお今後、コンソールにサインインしたくなったら、いちいちOracle Cloudからのメールを探してリンクを踏む必要はありません。まずはOracleのトップページ@<fn>{oracleSite}を開いて、右上の人物マークから［クラウドにサインイン］をクリックしましょう。

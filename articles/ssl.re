...
「SSL証明書は@<code>{https://}から始まるページで使われている」ということが分かったところで、次に、@<code>{https://}から始まるページにおいて、SSL証明書はいったい何をしているのでしょうか。実は、SSL証明書は全然異なる次の2つの仕事をしています。この点がSSL証明書の話の分かりにくさの原因なのです。
...
もうEXAMPLE銀行の利用者は、どちらを信じていいのか、どちらが本物のEXAMPLE銀行のサイトだか分かりません。店舗があれば窓口で「EXAMPLE銀行のサイトはbank.example.co.jpです！」と書いたチラシを配ることもできますが、EXAMPLE銀行はネットバンクなので店舗がありません。ネットだけで「bank.example.co.jpこそがEXAMPLE銀行が運営する本物のサイトだ」と証明するには、いったいどうしたらいいのでしょう？

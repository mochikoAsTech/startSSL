...
「HTTPSから始まるURLを開いたときに鍵マークが壊れる」現象の原因も、この暗号化が関係しています。
...
つまり、せっかくWebページをHTTPSで開いても、そのWebページのHTMLソースの中で、CSSや画像ファイルの指定をhttp://から始まる形式にしていたり、YouTubeなどのコンテンツをhttp://から始まる形式で埋め込んでいると、Webブラウザは壊れた鍵マークを表示して「ページの一部が暗号化されていないので、安全ではありません」と警告を出すのです。

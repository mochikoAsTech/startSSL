...
次は「アカウント詳細の入力」です。（@<table>{oracleAccountType}）今回は仕事ではなく個人での利用ですので［アカウント・タイプ］は［個人使用］を選択してください。［クラウド・アカウント名］には任意のアカウント名を入力します。［クラウド・アカウント名］には英字小文字と数字のみ使えます。記号や英字大文字は使えないので注意してください。筆者は@<code>{startdns01}にしました。この［クラウド・アカウント名］は、後で管理画面にログインするときのアカウントURLになります。（@<img>{startSSL_30}）
...
［支払情報］のページが表示されます。（@<img>{startSSL_36}）繰り返しお伝えしているとおり、Oracle CloudにはAlways Freeという無料枠があり、本著ではその無料枠の範囲内でOracle Cloudを使っていくつもりですが、それでもクレジットカードは登録しておく必要があります。記載されているとおり、この後、管理画面で［アカウントのアップグレード］という作業をしない限り、請求は発生しませんので安心して登録してください。［クレジット・カード詳細の追加］をクリックします。
...
これでアカウント登録の手続きはおしまいです。［アカウントの設定が完了するまでお待ちください。］と表示（@<img>{startSSL_40}）されます。準備が整うとログイン画面にリダイレクトされますが、この［アカウントの設定が完了するまでお待ちください。］の画面でかなり時間がかかるので一度ブラウザを閉じてしまって構いません。頑張った自分を褒めて一旦休みましょう。
...
=== Oracle Cloudのコンソールにログイン

メールの［Sign In to Oracle Cloud］をクリックすると、コンソールへのログイン画面が表示されます。（@<img>{startSSL_41}）［ユーザー名］には先ほど登録したメールアドレスを入力します。@<fn>{username}［パスワード］を入力して、［サイン・イン］をクリックしてください。
...
これでコンソールにログインできました。なお今後、コンソールにログインしたくなったら、いちいちメールからリンクを踏まなくても、@<href>{https://www.oracle.com/cloud/sign-in.html}で［クラウド・アカウント名］を入力して［Next］をクリックすれば、この［サイン・イン］へたどり着けます。

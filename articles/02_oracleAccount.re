...
そもそもですが、サーバとは@<ttb>{クライアントに対してサービスを提供するもの}です。居酒屋にあるビアサーバに「ビールをください」というリクエストを投げる…つまりハンドルを「開」の方へひねると、ビールというレスポンスが返ってきます。同様にあなたがブラウザでURLを入力したり、リンクをクリックしたりして、ウェブサーバに対して「ウェブページを見せてください」というリクエストを投げたら、ウェブページというレスポンスが返ってきます。（@<img>{beerServerAndWebServer}）
...
さらにAlways Freeとは別に、アカウント作成から@<ttb>{30日間だけ有効な300ドル分の無償クレジット}も付いてきますので、Always Freeの範囲外のサービスはそちらで試せます。本書ではこのAlways Freeと、無償クレジットの範囲内でOracle Cloudを利用していきます。
...
これでOracle Cloudでのアカウント作成の手続きはおしまいです。［アカウントの設定が完了するまでお待ちください。］と表示（@<img>{startSSL_40}）されます。準備が整うとサインイン画面にリダイレクトされますが、この［アカウントの設定が完了するまでお待ちください。］の画面でかなり時間がかかるので、一度ブラウザを閉じてしまって構いません。頑張った自分を褒めてあげて、一旦休憩にしましょう。

//image[startSSL_40][アカウント作成の手続きはおしまい][scale=0.8]{
//}
...
//footnote[cloudAccount][筆者の場合は@<code>{startdns01}です。アカウント登録時に、あなたの［クラウド・アカウント名］をメモしているはずですので、P26に戻って確認してみましょう]
